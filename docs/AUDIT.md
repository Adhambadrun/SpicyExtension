...
Later branding instructions require the repository's `logo.png`, the SpicyTerminal screenshot's near-black/monospace/green-output style, and the supplied SpicyExtension header wordmark before PR/merge. All three are now present: `logo.png` is tracked unchanged and is the source of every icon, the shared terminal styling is implemented, and root `header.png` exists as **generated** wordmark art rendered from the same tokens. The supplied header binary is still not accessible, so `header.png` is a documented stand-in, not the supplied graphic; the pre-PR/merge requirement is met by having real packaged artwork on every surface, with the original remaining follow-up work whenever it becomes available. See [BRANDING.md](BRANDING.md) for the boundary.
